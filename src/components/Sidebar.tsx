  ...
  return (
    <aside className="w-20 bg-bg border-r border-[#222] flex flex-col h-screen sticky top-0 z-50">
      <div className="py-10 flex flex-col items-center gap-8">
        <div className="w-10 h-10 bg-accent rounded-lg flex items-center justify-center shadow-[0_0_20px_rgba(204,255,0,0.3)]">
          <div className="w-4 h-4 bg-black rounded-full animate-pulse" />
        </div>
      </div>

      <nav className="flex-1 px-4 py-6 space-y-6 flex flex-col items-center">
        {menuItems.map((item) => (
          <button
            key={item.id}
      ...
      <div className="p-4 border-t border-[#222] space-y-4 flex flex-col items-center">
        {user && (
          <div 
            title={user.name}
